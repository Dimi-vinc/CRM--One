...
export function SuperAdminDashboard() {
  ...
  const cards = [
    { l: 'Tenants', v: String(filteredTenants.length), i: Building2, c: 'blue' as const },
    { l: 'Utilisateurs', v: String(totalUsers), i: Users, c: 'teal' as const },
    { l: 'MRR (USD)', v: fmt(mrrUsd, 'USD'), i: DollarSign, c: 'orange' as const },
    { l: 'Ventes tracées', v: fmt(totalSales, 'USD'), i: TrendingUp, c: 'violet' as const },
  ];

  return (
    <div>
      <PageHeader title={`Super Admin — ${profile?.email}`} subtitle="Vue transverse sur tous les tenants" />
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <Card className="p-5 lg:col-span-2">
          <h3 className="text-base font-semibold text-gray-900">Répartition par forfait</h3>
        ...
        <Card className="p-5">
          <h3 className="text-base font-semibold text-gray-900 flex items-center gap-2"><Globe2 size={16} className="text-mint-600" /> Top pays</h3>
          <div className="mt-3 space-y-2">
            ...
            {byCountry.length === 0 && <p className="text-sm text-gray-400">Aucune donnée</p>}
          </div>
        </Card>
      </div>

      <Card className="mt-6 overflow-hidden">
        <div className="border-b border-gray-100 px-5 py-3"><h3 className="text-sm font-semibold text-gray-900">Tenants récents</h3></div>
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-left text-xs uppercase text-gray-500"><tr><th className="px-4 py-2.5">Entreprise</th><th className="px-4 py-2.5">Pays</th><th className="px-4 py-2.5">Plan</th><th className="px-4 py-2.5">Statut</th><th className="px-4 py-2.5">Créé</th></tr></thead>
